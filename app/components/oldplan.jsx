...
function assignGuestsToHosts(hosts, guests) {
  ...
  allergicGuests.forEach((guest) => {
    // getAllergyFreeMeal(g.allergy);
    // console.log(assignAllergicGuestsToMeal(guest));
    const meals = assignAllergicGuestsToMeal(guest);
    // console.log(guest.allergy, meals);
  });
  ...
  function assignAllergicGuestsToMeal(guest) {
    ...
    let appetizer = null,
      dinner = null,
      dessert = null,
      allergy = guest.allergy ? guest.allergy.toLowerCase() : "";

    // Iterate over hosts
    hosts.forEach((host) => {
      if (host.appetizer_allergy.split(",").includes(allergy) && !appetizer) {
        appetizer = getAvailableHost(allergy, "appetizer_allergy");
      }
      if (host.dinner_allergy.split(",").includes(allergy) && !dinner) {
        dinner = getAvailableHost(allergy, "dinner_allergy");
      }
      if (host.dessert_allergy.split(",").includes(allergy) && !dessert) {
  ...
  function getAvailableHost(allergy, mealType) {
    let arr = hosts.filter((h) => !h[mealType].split(",").includes(allergy));
    console.log(allergy, h);
